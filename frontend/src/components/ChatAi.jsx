...
const ChatAi = () => {
  const [messages, setMessages] = useState([
    {
      ...
      type: "assistant",
      content:
        "Xin chào! Tôi là AI thời đại mới. Tôi có thể giúp gì cho bạn hôm nay?",
      timestamp: new Date(),
    },
  ]);
  ...
  const BASE_URL =
    import.meta.env.MODE === "development"
      ? "http://localhost:5000"
      : "https://chat-ai-backend-silk.vercel.app";
  ...
  // ✅ Stream API
  const callBackendStream = async (question, onChunkStart, onChunk) => {
    ...
    while (true) {
      ...
      if (firstChunk && chunk.trim()) {
        onChunkStart(); // ✅ báo cho frontend tạo assistant message & tắt typing
        firstChunk = false;
      }
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      await callBackendStream(
        userMessage.content,
        () => {
          // ✅ khi có chunk đầu tiên: tắt typing + thêm assistant message rỗng
          setIsTyping(false);
          setMessages((prev) => [
            ...prev,
            { id: aiMessageId, type: "assistant", content: "", timestamp: new Date() },
          ]);
        },
        (chunk) => {
          // ✅ append text vào assistant message
          setMessages((prev) =>
            prev.map((m) =>
